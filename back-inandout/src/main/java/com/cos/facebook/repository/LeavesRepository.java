...
public interface LeavesRepository extends JpaRepository<Leaves, Integer>{
	...
	@Query(value = "select * from Leaves where state='success' and (category='연차' or category='오전 반차' or category='오후 반차' or category='출장' or category='외근')",nativeQuery = true)
	public List<Leaves> findByNo234();
	
	@Query(value = "select * from Leaves where state='success' and (category='연차' or category='오전 반차' or category='오후 반차' or category='출장')",nativeQuery = true)
	public List<Leaves> findByNo23();
	
	@Query(value = "select * from Leaves where state='success' and (category='연차' or category='오전 반차' or category='오후 반차' or category='외근')",nativeQuery = true)
	public List<Leaves> findByNo24();
	
	
	@Query(value = "select * from Leaves where (state='success' and userId=:id) or (state='success' and (category='연차' or category='오전 반차' or category='오후 반차'))",nativeQuery = true)
	...
	@Query(value = "select * from Leaves where (state='success' and userId=:id) or (state='success' and (category='연차' or category='오전 반차' or category='오후 반차')) or category='출장'",nativeQuery = true)
	public List<Leaves> findByNo123(long id);
	@Query(value = "select * from Leaves where (state='success' and userId=:id) or (state='success' and (category='연차' or category='오전 반차' or category='오후 반차')) or category='외근'",nativeQuery = true)
