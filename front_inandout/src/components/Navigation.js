...
const Navigation = (name) => {
    ...
    const 네비상태 = useSelector( (state) => state );
    ...
    return (
        ...
        <Sider width={200} className="site-layout-background2">
                ...
                <Menu
                    ...
                    // openKeys={openKeys}
                    // openKeys={네비상태.defaultOpenKeys}
                    // selectedKeys={selectedKeys}
                    // onOpenChange={(openKeys) => setOpenkeys(openKeys)}
                    // onOpenChange={() => {dispatch({type : '증가'})}}
                    // defaultOpenKeys={['1', 'sub2', 'sub3', 'sub4', 'sub5', 'sub6']}
                    defaultOpenKeys={네비상태.defaultOpenKeys}
                >
                    <Menu.Item key="1" icon={<HomeOutlined />} onClick={() => {dispatch({type : ''})}}>
                        <Link to="/">홈화면</Link>
                    </Menu.Item>
                    <SubMenu key="sub2" icon={<UserOutlined />} title="내 정보" onClick={() => {dispatch({type : '내 정보'})}}>
                        <Menu.Item key="5"><Link to="/mypage">프로필 수정</Link></Menu.Item>
                    </SubMenu>
                    <SubMenu key="sub3" icon={<LaptopOutlined />} title="내 근무" onClick={() => {dispatch({type : '내 근무'})}}>
                        {/* <Menu.Item key="6"><Drawer /></Menu.Item> */}
                        <Menu.Item key="7"><Link to="/work">근무 현황</Link></Menu.Item>
                    </SubMenu>
                    <SubMenu key="sub4" icon={<FileSearchOutlined />} title="의원 관리" onClick={() => {dispatch({type : '의원 관리'})}}>
                        <Menu.Item key="8"><Link to="/him">의원 정보 관리</Link></Menu.Item>
                        <Menu.Item key="9"><Link to="/wtm">근무시간 관리</Link></Menu.Item>
                    </SubMenu>
                    <SubMenu key="sub5" icon={<IdcardOutlined />} title="사원 관리" onClick={() => {dispatch({type : '사원 관리'})}}>
                    ...
                    <SubMenu key="sub6" icon={<NotificationOutlined />} title="공지사항" onClick={() => {dispatch({type : '공지사항'})}}>
            ...
            <Sider width={200} className="site-layout-background2">
                ...
                >
                    <Menu.Item key="1" icon={<LaptopOutlined />} onClick={() => {dispatch({type : ''})}}>
                        <Link to="/">홈화면</Link>
                    </Menu.Item>
                    <SubMenu key="sub2" icon={<UserOutlined />} title="내 정보" onClick={() => {dispatch({type : '내 정보'})}}>
                        <Menu.Item key="5"><Link to="/mypage">프로필 수정</Link></Menu.Item>
                    </SubMenu>
                    <SubMenu key="sub3" icon={<NotificationOutlined />} title="내 근무" onClick={() => {dispatch({type : '내 근무'})}}>
                        <Menu.Item key="6"><Drawer /></Menu.Item>
                        <Menu.Item key="7"><Link to="/work">근무 현황</Link></Menu.Item>
                    </SubMenu>
                    <SubMenu key="sub4" icon={<FileSearchOutlined />} title="의원 정보" onClick={() => {dispatch({type : '의원 정보'})}}>
